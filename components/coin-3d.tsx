...
export type Coin3DProps = {
  ...
  roughness?: number;
  spinSpeed?: number;
  frontImage?: string; // front image path
  backImage?: string; // back image path
};
...
export default function Coin3D({
  ...
  useEffect(() => {
    ...
    if (frontImage) {
      ...
      faceFront = new THREE.Mesh(
        new THREE.CircleGeometry(radius - 0.02, 96),
        matFront
      );
    } else {
      const matFront = new THREE.MeshStandardMaterial({
    ...
    // Back face
    let faceBack: THREE.Mesh;
    if (backImage) {
      const texLoader = new THREE.TextureLoader();
      const backTex = texLoader.load(backImage);
      const matBack = new THREE.MeshStandardMaterial({
        map: backTex,
        metalness,
        roughness,
      });
      faceBack = new THREE.Mesh(
        new THREE.CircleGeometry(radius - 0.02, 96),
        matBack
      );
    } else {
    ...
    const ringFront = new THREE.Mesh(
      new THREE.RingGeometry(radius * 0.78, radius * 0.92, 96),
      accentMat
    );
  ...
  }, [metalness, roughness, spinSpeed, frontImage, backImage]);
